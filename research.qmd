...
for _, data in yaml_data.items():
    ...
    authors = [aut if aut != "me" else "Michael Schulte-Mecklenbeck" for aut in authors]
    ...
    if paper is not None:
        buttons.append(button(paper, "Paper", "bi-file-earmark-pdf"))
    ...
    osf = data.get("osf")
    if osf is not None:
        buttons.append(button(osf, "OSF", "bi-folder2-open"))

    pub_url = data.get("published_url")
    venue = data.get("venue")
    working_paper = pub_url is None
    
    pub_str = f'{author_str.rstrip(".")}. ({year_str}). <strong>{title_str}</strong>.'
    ...
    doi = data.get("doi")
    if doi is not None:
        pub_str += f' <a href="https://doi.org/{doi}" target="_blank" rel="noopener noreferrer">https://doi.org/{doi}</a>'
