...
We ran a direct replication of experiment 1 of Nguyen et al. (2022) and developed a set of six new tasks (see [Appendix](#appendix)) that structurally followed the original: two numbers (either `round` or `precise`) and two additional numbers that were not relevant for the task.
...
So, clearly we produced two, quite different sets of stimuli. Set 1 had a average of `r mean(targets[targets$set == 1 & targets$number_id == 1,]$target,1)` for the first number and `r mean(targets[targets$set == 1 & targets$number_id == 2,]$target,1)`, whereas set 2 had `r mean(targets[targets$set == 2 & targets$number_id == 1,]$target,1)` for the first  and `r mean(targets[targets$set == 2 & targets$number_id == 2,]$target,1)` for the second number. Let's see whether these differences will result in the same set of resutls.

# Demographics

The original paper had 1300 participants recruited on Amazon’s Mechanical Turk. We recruited through word of mouth among friends and family of the student group running this study, resulting in a substaintially smaller sample.  
...
Let's first look at the overall effect of the manipulation (see Figure @fig-overview). Applying the same rule as Nguyen et al. (2022) we code all responses that fall within a 10% of the precise number as a correct response. Looking at the percentage of correct responses between the precise (`r prop_table_prec[1,2]`) and round (`r prop_table_prec[2,2]`) condition we see that the round condition has a ~`r prop_table_prec[2,2] - prop_table_prec[1,2]*100`% higher percentage of correct responses than precise. 
...
```{r}
res <- prop.test(x = c(prop_table$n[2], prop_table$n[4]), n = c(prop_table$n[1] + prop_table$n[2], prop_table$n[3] + prop_table$n[4]))
# Printing the results
res 

checked_df %>%
  filter(test == 'Question') %>%
t.test(check_within_range ~ condition, data = .)
