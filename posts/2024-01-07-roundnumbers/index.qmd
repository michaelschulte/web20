...
We ran a direct replication of experiment 1 of Nguyen et al. (2022). We developed a set of six new tasks (see [Appendix](#appendix)) that structurally followed the original: two numbers (either `round` or`precise`) and two additional numbers that were not relevant for the task.
...
So, clearly we produced two, quite different sets of stimuli. Set 1 had a average of `r mean(targets[targets$set == 1 & targets$number_id == 1,]$target,1)` for the first number and `r mean(targets[targets$set == 1 & targets$number_id == 2,]$target,1)`, whereas set 2 had `r mean(targets[targets$set == 2 & targets$number_id == 1,]$target,1)` for the first  and `r mean(targets[targets$set == 2 & targets$number_id == 2,]$target,1)` for the second number. 
