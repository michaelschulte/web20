...
There are several differences inspecting our results. Our rank ordered results give us the pattern: **612534**. Remarkable are also the much lower participation rates (below 50%) for `Not participate` and `No Default` in both framing conditions. Only `Participate` in the positive frame is above 75% (see @fig-results-exp2).
...
- [`BAFS2023_exp1_pub.csv`](https://github.com/michaelschulte/web20/blob/main/0_data/BAFS2023_exp1_pub.csv) and [`BAFS2023_exp2_pub.csv`](https://github.com/michaelschulte/web20/blob/main/0_data/BAFS2023_exp2_pub.csv): one row per participant and task, with the `Default_condition`, the `Framing`, the `Answer_Option` shown and the `choice` made.
- [`BAFS2023_demographie_pub.csv`](https://github.com/michaelschulte/web20/blob/main/0_data/BAFS2023_demographie_pub.csv): `education`, `gender` and `agegroup` per participant. Exact age is held back; only the age group is published.
