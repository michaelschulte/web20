...
- [`BAHS2021_raw_public.csv`](https://github.com/michaelschulte/web20/blob/main/0_data/BAHS2021_raw_public.csv): one row per participant, keyed on a random `ID`. The 45 binary choices of each condition sit in `L1a_*` (names), `Lee2_*` (pictures), `Lee3_*` (color) and `Lee4_*` (B&W), the knowledge questions in `L_KNOWLEDGE_*` and `KNOW_*`, a separate choice and willingness-to-pay block in `A0_*` to `A6_*`, the product ratings in `Rating_Product_*`, and the per-item click counts and page times in the `TIMING_*` columns. `Gender` and `Age` are included. Respondent IP addresses and coordinates were part of the Qualtrics export and have been removed.
- [`BAHS2021_combinations.csv`](https://github.com/michaelschulte/web20/blob/main/0_data/BAHS2021_combinations.csv): the experimental plan -- for each `QuestionId`, the two items that were compared (`combi1`, `combi2`).
