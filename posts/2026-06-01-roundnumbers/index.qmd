...
The overall effect is encouraging, but our two task sets differed considerably in the magnitude of the numbers used (see Appendix). Set 1 covered large administrative numbers (unemployment figures, court judgements, geographic areas), while Set 2 used more everyday-scale numbers (doctor's offices, tunnel length, wages). If the round-number advantage is robust, it should appear in both sets — though we might expect set-specific variation given how differently the numbers were rounded (e.g., 103,156 → 100,000 is a coarser rounding than 16,942 → 17,000).

@fig-split shows the condition effect separately for each task set.
...
Beyond the set-level split, each individual number carries its own character — the precision of the rounding, the familiarity of the domain, and the scale of the value. @fig-facet breaks down performance by every individual number within each set and condition, giving us the most granular view of where the round-number benefit is clearest and where it is weakest or even absent.

This level of detail is useful for two reasons. First, it reveals whether a single hard or easy item is driving the overall result. Second, it allows us to inspect whether specific stimuli (e.g., numbers that were barely rounded vs. heavily rounded) behave differently — which would have implications for how future studies design their stimuli.
...
The anonymised responses behind every figure and table on this page, each file keyed on the participant's random `ID`:

- [`BAHS2023_complete_df_pub.csv`](https://github.com/michaelschulte/web20/blob/main/0_data/BAHS2023_complete_df_pub.csv): one row per participant, task and number, with the `task`, the `condition` (`Round` or `Precise`), whether the row comes from the recall or the estimation part (`test`), which of the two numbers it refers to (`number_id`), the `value` given and the `target` it should approximate.
- [`BAHS2023_targets_pub.csv`](https://github.com/michaelschulte/web20/blob/main/0_data/BAHS2023_targets_pub.csv): the stimulus values, one row per task and number -- the `target`, its `set` and `label` and the `range_min`/`range_max` of the answer scale.
- [`BAHS2023_demographie_pub.csv`](https://github.com/michaelschulte/web20/blob/main/0_data/BAHS2023_demographie_pub.csv): year of birth (`Jahrgang`), gender (`Geschlecht`) and age (`Alter`) per participant.
