...
date: "2024-05-27"
draft: false
categories: [Replication, Decision Making]
sidebar:
  contents: auto
editor_options: 
...
$$CompletionTime_{i} < M_{CompletionTime} + 3SD_{CompletionTime}$$ This step gave us a final sample of *n* = `r dim(raw)[1]` participants. Due to my oversight, we did not collect gender or age information.
...
Inspecting the results in Tong et al. (2013) the following patterns were found. I will simply summarize the choices in the money priming condition where in Exp 1: 70 % / Exp 2: 57 % / Exp 3: 77 % of participants choose the hedonic option compared to Exp 1: 40 % / Exp 2: 30 % / Exp 3: 55 % in the control condition). So overall sizable differences in the choice patterns between 22 and 30 points difference between the conditions.
...
Another small oversight was the addition of a control condition in Tong in Experiment 2 where they added an 'empty' control task, where no identification of currency or foreign language was included. I believe this is a smaller issue.

Overall this was a fun exercise, a good teaching experience (in a rather large group).

## Within versus between subjects - design {#sec-order}

Here I add some additional analysis, checking for an effect of *Order* - which is the variation of whether Tong or Caruso appeared first for participants (as discussed above).
...
The anonymised responses behind every figure and table on this page:

- [`BAFS2024_complete_df_pub.csv`](https://github.com/michaelschulte/web20/blob/main/0_data/BAFS2024_complete_df_pub.csv): the questionnaire as exported, one row per participant, with the priming `Condition` and the `Order` the scales were shown in.
- [`BAFS2024_scale_av.csv`](https://github.com/michaelschulte/web20/blob/main/0_data/BAFS2024_scale_av.csv): the per-participant scale averages used in the analysis -- `av_SDO`, `av_BJW` and `av_SJS` -- alongside `Condition` and `Order`.
- [`BAFS2024_Saetze_Manipulation.txt`](https://github.com/michaelschulte/web20/blob/main/0_data/BAFS2024_Saetze_Manipulation.txt): the sentences used in the scrambling task, with `sentence_id` marking the control and treatment sets.
